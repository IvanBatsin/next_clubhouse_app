import React from 'react';
import Button from '../../components/Button/Button';
import StepHeader from '../../components/StepHeader';
import WhiteBlock from '../../components/WhiteBlock';
import { useMainContext, UserData } from '../../pages';
import styles from './GitHubStep.module.scss';
import Cookies from 'js-cookie';

const GitHubStep: React.FC = () => {
  const { onNextStep, setUserData } = useMainContext();

  const onClickAuth = (): void => {
    window.open(
      'http://localhost:4000/auth/github/',
      'Auth', 
      'width=400,height=400,status=yes,toolbat=no,menubar=no,location=no'
    );
  }

  React.useEffect(() => {
    window.addEventListener('message', (event) => {
      if (typeof event.data === 'string' && event.data.includes('avatarUrl')) {
        const data: UserData = JSON.parse(event.data);
        setUserData!(data);
        Cookies.set('token', data.token);
        onNextStep!();
      }
    });
  }, []);

  return (
    <div>
      <StepHeader>
        <img src="data:image/svg+xml;base64," />
        <h4>Do you want import info from GitHub</h4>
      </StepHeader>
      <WhiteBlock>
        <div className={styles.content}>
          <div className={styles.avatar}>
            AD
          </div>
          <span className={styles.username}>User name</span>
          <Button onClick={onClickAuth}>Import from GitHub &rarr;</Button>
          <span>Enter my info manually</span>
        </div>
      </WhiteBlock>
    </div>
  )
}

export default GitHubStep;